...
from Device_bqfl_avg import Device, DevicesInNetwork
...
if __name__=="__main__":
    ...
    if args['resume_path']:
        ...
        devices_in_network = DevicesInNetwork(data_set_name='mnist',
                                              is_iid=args['IID'],
        ...
        for comm_round in range(latest_round_num + 1, args['max_num_comm']+1):
            ...
            for node, worker in enumerate(workers_this_round):
                ...
                with open("data.txt","a") as dfile:
                    dfile.write(f"{node} - COMM: {comm_round} - Worker ID: {worker.return_idx()} - Data: {y_train_cat}\n")
                data = tf.data.Dataset.from_tensor_slices((x_train_node, y_train_node)).batch(128)
                worker.train_dl = data
                key, subkey = jax.random.split(key)
                params = jax.random.normal(subkey, (3 * k, n))
                # THis is for initial params, after 1 comm round, params will be updated
                if comm_round == 1:
                    worker.global_params = params

            for device in devices_list:
                device.x_test = x_test
                device.y_test = y_test
            ...
            for validator_iter in range(len(validators_this_round)):
                ...
                f123 = open("worker_local_update_check.txt", "a")
                if args['miner_acception_wait_time']:
                    ...
                    for worker_iter in range(len(associated_workers)):
                        ...
                        if not worker.return_idx() in validator.return_black_list():
                            ...
                            while total_time_tracker < validator.return_miner_acception_wait_time():
                                ...
                                if worker.online_switcher():
                                    local_update_spent_time, params, acc_list, loss_list = worker.worker_local_update(rewards,
                                                                                         log_files_folder_path_comm_round,
                                                                                         comm_round)
                                    f123.write(f"Updating in first Section by Worker {worker.return_idx()}")
                    ...
                    for worker_iter in range(len(associated_workers)):
                        ...
                        if not worker.return_idx() in validator.return_black_list():
                            print(f'worker {worker_iter+1}/{len(associated_workers)} of validator {validator.return_idx()} is doing local updates')
                            if worker.online_switcher():
                                local_update_spent_time, params, acc_list, loss_list = worker.worker_local_update(rewards,log_files_folder_path_comm_round, comm_round, local_epochs=args['default_local_epochs'])
            ...
            for i, worker in enumerate(workers_this_round):
                params_list.append(worker.global_params)
            ...
            tqdm.write(f'world {comm_round}: test acc={test_acc:.4f}, test loss={test_loss:.4f}')

            with open(f"world_test.txt", "a") as file:
                file.write(
                    f'No of Classes: {n_class} - COMM: {comm_round} -  test acc={test_acc:.4f}, test loss={test_loss:.4f}\n')
                ...
                try:
                    ...
                    with open(f"{log_files_folder_path}/forking_and_no_valid_block_log.txt", 'a+') as file2:
                        ...
                        try:
                            ...
                            file2.write(no_valid_block_msg+"Except")
