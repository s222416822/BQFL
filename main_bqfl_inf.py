...
from Device_bqfl_inf import Device, DevicesInNetwork
...
parser.add_argument('-max_ncomm', '--max_num_comm', type=int, default=10,
...
if __name__ == "__main__":
    ...
    if args['resume_path']:
        ...
        for device in devices_list:
            ...
            # device.init_global_parameters(key, k, n)
    ...
    # VBFL starts here
        for comm_round in range(latest_round_num + 1, args['max_num_comm'] + 1):
            ...
            for node, worker in enumerate(workers_this_round):
                ...
                worker.opt_state = opt.init(params)
            ...
            for i, worker in enumerate(workers_this_round):
                params_list.append(worker.local_train_params)
                gmm_list.append(worker.gmm)

            test_acc = jnp.mean(
                pred_comb(params_list, x_test[:1024], k, d_mode='GMM').argmax(axis=-1) == y_test[:1024].argmax(axis=-1))
            ...
            tqdm.write(f'world {0}: test acc={test_acc:.4f}, test loss={test_loss:.4f}')
