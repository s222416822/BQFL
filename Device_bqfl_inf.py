...
class Device:
    ...
    def worker_local_update(self, rewards, log_files_folder_path_comm_round, comm_round, local_epochs=1):
        ...
        for epoch in tqdm(range(local_epochs), leave=False):
            for i, (x, y) in enumerate(self.train_dl):
                x = x.numpy()
                y = y.numpy()
                loss_val, grad_val = self.compute_loss(self.params, x, y, k)
                updates, self.opt_state = self.opt.update(grad_val, self.opt_state, self.params)
                self.params = optax.apply_updates(self.params, updates)
                loss_mean = jnp.mean(loss_val)
                if i % 20 == 0:
                    acc = jnp.mean(self.compute_accuracy(self.params, x, y, k))
                    acc_list.append(acc)
            ...
            with open(f"{log_files_folder_path_comm_round}/worker_{self.idx}_{is_malicious_node}_local_updating_accuracies_comm_{comm_round}.txt",
                    "a") as file:
                file.write(f"{self.return_idx()} epoch_{epoch + 1} {self.return_role()} {is_malicious_node}: {self.validate_model_weights(self.params)}\n")
        ...
        with open(f"{log_files_folder_path_comm_round}/worker_final_local_accuracies_comm_{comm_round}.txt",
                  "a") as file:
            file.write(
                f"{self.return_idx()} {self.return_role()} {is_malicious_node}: {self.validate_model_weights(self.params)}\n")
        print(f"Done {local_epochs} epoch(s) and total {self.local_total_epoch} epochs")
        # self.local_train_parameters = self.net.state_dict()
        self.local_train_params = self.params
        return self.local_update_time, self.params, acc_list, loss_list
    ...
    def global_update(self, local_update_params_potentially_to_be_used):
        ...
        if not local_params_by_benign_workers == None:
            ...
            self.updated_params_list = local_params_by_benign_workers  #Whole arraylist stored for now
    ...
    def validator_update_model_by_one_epoch_and_validate_local_accuracy(self, opti):
        ...
        if self.computation_power == 0:
            ...
            # no validation mechanism for now
